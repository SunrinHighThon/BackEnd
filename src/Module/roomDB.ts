...
function leave(data){
    ...
    Roomdb.get('RoomData').find({_id:data._id}).assign({connectedUsers : people}).write()
   
    Roomdb.get('RoomData').find({_id:data._id}).assign({player:UpdatePlayer}).write()
...
export default {setting,push,remove,join,searchAll,leave}
